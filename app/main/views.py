...
import datetime,time,random,uuid
from .objects import AddObjectForm
from ..models import *
from werkzeug.datastructures import CombinedMultiDict, MultiDict,ImmutableDict
...
def management_3(sys_id,object_id):
    ...
    page = request.args.get('page', 1, type=int)
    pagination = aop_model_score.query.filter_by(sys_id=sys_id,object_id=object_id).order_by(aop_model_score.score).paginate(
        page, per_page=current_app.config['FLASKY_POSTS_PER_PAGE'],
        error_out=False)
    posts = pagination.items
    system_list, object_list, model_list = left_nav()
...
def add_manage():
    ...
    if form.validate_on_submit():
        ...
        object = aop_object_score.query.filter_by(sys_id = sys_uuid.hex, object_id=form.ip.data, object_type=form.type.data).first()
        if object is None:
...
def report():
    ...
    for i in range(day_range*24):
            #字符串格式的搜索开始时间
        current_hour_range = (datetime.datetime.strftime((search_range + datetime.timedelta(hours=i+1)),"%Y-%m-%d %H:00:00"))
            #向柱状图X轴列表写入信息
        xAxisData.append(current_hour_range)

        #初始化判断条件及累加器
        # judge = current_time.hour+1
    xAxisData_first = datetime.datetime.strptime(xAxisData[0],"%Y-%m-%d %H:%M:%S")
    intital_value = 0
    intital_length = len(xAxisData)
    dataOK = [intital_value]*intital_length
    dataPB = [intital_value]*intital_length

        #遍历所有数据，求取柱状图数据
        #此处不能用小时数遍历。因为无法保证sqlalchemy的order_by对于datetime格式的数据有效
    for j in bar_data:
        # 获取时间差(小时),转换成int时，会自动保留整数位
        time_number = int((j.time - xAxisData_first).total_seconds() / 3600)
        if j.status == 'OK':
            dataOK[time_number] = dataOK[time_number] + 1
        else:
            dataPB[time_number] = dataPB[time_number] + 1

        #插件筛选范围的区间
    maxNum = max(dataOK+dataPB)
    ...
    return render_template('report.html',system_list=system_list, object_list=object_list, model_list=model_list,
                           ...
                           xAxisData=xAxisData,dataPB=dataPB,dataOK=dataOK,maxNum=maxNum)
...
def usercontrol():
    if current_user.is_administrator:
        if request.method == 'GET':
            ...
            return render_template('auth/../templates/kaiquanxian/usercontrol.html', role_list=role_list, user_list=user_list)
        else:
            form = request.form
            ch_list = []
            print(form)
